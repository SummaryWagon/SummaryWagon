...
const ArticleKeys = {
  ...
  lists: () => [...ArticleKeys.all, "historyArticles"] as const,
  list: (filters: string) => [...ArticleKeys.lists(), { filters }] as const,
};

export const useFetchHistoryArticles = (size: number, email: string) => {
  const getBoardPage = async ({ pageParam = 1 }) => {
    const url = `${process.env.NEXT_PUBLIC_SERVER_URL}/articles/all?email=${email}&page=${pageParam}&limit=${size}`;
